module attributes {transform.with_named_sequence} {
  ...
  %relu = transform.structured.match 
      attributes {op_label = "relu"} in %arg0 : (!transform.any_op) -> !transform.any_op
  // transform.print %relu : !transform.any_op
  ...
  // 对relu进行分块 (n128, m96)
  %relu_n, %for3_n = transform.structured.extended_tile_using_for %relu
      tile_sizes [0, 128]
      : (!transform.any_op) -> (!transform.any_op, !transform.any_op)

  %relu_n_m, %for3_m = transform.structured.extended_tile_using_for %relu_n
    tile_sizes [96]
    : (!transform.any_op) -> (!transform.any_op, !transform.any_op)

  %add_result_ddr = transform.get_operand %relu_n_m[1] : (!transform.any_op) -> !transform.any_value
