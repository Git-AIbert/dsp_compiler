    # -debug-only=schedule-transform-op \
    # -debug-only=consumer-fusion \
    # -debug-only=deduplicate-multi-buffer \
    # -debug-only=chain-split-reduction-pipelines \
    # -debug-only=buffer-loop-sinking \
    # tile_fuse_consumer

../../build/bin/cascade-opt \
    matmul_add_relu.mlir \
